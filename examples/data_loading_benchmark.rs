...
fn main() -> Result<()> {
    ...
    println!("   ✓ Loaded {} rows, {} columns in {:?}", df.height(), df.width(), votable_time);
    ...
    println!("   ✓ Estimated load time: {:?} (vs {:?} for VOTable)", estimated_parquet_time, votable_time);
    
    if votable_time.as_secs_f64() > 0.0 {
        let speedup = votable_time.as_secs_f64() / estimated_parquet_time.as_secs_f64();
        println!("   ✓ Estimated speedup: {:.1}x faster than VOTable", speedup);
    }
    ...
    for limit in [100, 1000, 10000] {
        let start = Instant::now();
        let df_subset = load_data_with_limit("data/stellarhosts.vot", Some(limit))?;
        let time = start.elapsed();
        println!("   ✓ {} rows: {:?} ({:.2} MB/row)", 
            df_subset.height(), 
            time,
            df_subset.estimated_size() as f64 / df_subset.height() as f64 / (1024.0 * 1024.0));
    }
    ...
    let key_columns = ["st_teff", "st_mass", "st_rad", "disc_year", "discoverymethod"];
    ...
    for &col_name in &key_columns {
        if let Ok(col) = df.column(col_name) {
            ...
            println!("{:<16} | {:>5} | {:>7} | {:>8.1}%", col_name, total, missing, missing_pct);
        } else {
            println!("{:<16} | {:>5} | {:>7} | {:>8}", "Column not found", "-", "-", "");
        }
    }
    ...
    if let Ok(disc_year_col) = df.column("disc_year") {
        if let Some(series) = disc_year_col.as_series() {
            if let Ok(years) = series.f64() {
                ...
                if total_years > 0 {
                    let min_year = years.into_iter().filter_map(|x| x).fold(f64::INFINITY, |a, b| a.min(b));
                    let max_year = years.into_iter().filter_map(|x| x).fold(f64::NEG_INFINITY, |a, b| a.max(b));
    ...
    if let (Ok(disc_year_col), Ok(hostname_col)) = (df.column("disc_year"), df.column("hostname")) {
        if let (Some(year_series), Some(host_series)) = (disc_year_col.as_series(), hostname_col.as_series()) {
            if let (Ok(years), Ok(hosts)) = (year_series.f64(), host_series.str()) {
                ...
                println!("  Valid discovery year records in first 10: {}", valid_records);
    ...
    println!("\n=== Recommendations ===");
    println!("1. Convert VOTable to Parquet for {:.0}x faster loading", votable_time.as_secs_f64() / estimated_parquet_time.as_secs_f64());
