...
fn main() -> Result<()> {
    ...
    let discovery_columns: Vec<String> = columns.iter()
        .filter(|&&name| name.contains("disc") || name.contains("year") || name.contains("method"))
    ...
    let date_columns: Vec<String> = columns.iter()
    ...
    let stellar_props = ["st_teff", "st_mass", "st_rad", "st_lum", "st_logg", "st_met", "st_age"];
    ...
    let planet_count_columns: Vec<String> = columns.iter()
    ...
    println!("Missing discovery columns: {}", !discovery_columns.is_empty());
    
    // Look for the most likely discovery-related column names
    let likely_discovery = columns.iter()
    ...
    let year_columns: Vec<String> = columns.iter()
...
fn print_sample_values(_name: &str, col: &polars::prelude::Column) {
    if let Some(series) = col.as_series() {
        if series.dtype().is_numeric() {
            if let Ok(f64_series) = series.f64() {
                let values: Vec<String> = f64_series.into_iter()
                    .take(5)
                    .map(|x| x.map(|v| format!("{:.3}", v)).unwrap_or_else(|| "NULL".to_string()))
                    .collect();
                println!("    → Sample values: {}", values.join(", "));
            }
        } else if let Ok(str_series) = series.str() {
            let values: Vec<&str> = str_series.into_iter()
                .take(5)
