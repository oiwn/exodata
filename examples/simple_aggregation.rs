...
use anyhow::Result;
use polars::prelude::{col, IntoLazy, ChunkAgg, ChunkVar, lit, SortMultipleOptions};
...
fn main() -> Result<()> {
    ...
    for decade in &discovery_data {
        let median_temp_str = decade.median_temp
            .map(|t| format!("{:.0}K", t))
            .unwrap_or_else(|| "N/A".to_string());
        println!("{:>5}s   | {:>5} | {}", 
            decade.decade, 
            decade.stars_discovered, 
            median_temp_str);
    }
    
    let total_discovered: u32 = discovery_data.iter().map(|d| d.stars_discovered).sum();
    println!("\nTotal Stars Discovered: {} | Computed in {:?}", 
        total_discovered, discovery_time);
    
    // Additional analysis: Discovery methods
    println!("\n=== DISCOVERY METHODS ===");
    if let Ok(discovery_methods_df) = df.clone().lazy()
        .group_by(["discoverymethod"])
        .agg([
            lit(1).count().alias("count")
        ])
        ...
        .collect() 
    {
        ...
        for row in 0..discovery_methods_df.height() {
            let method = discovery_methods_df.column("discoverymethod")?.str()?.get(row).unwrap_or("N/A");
            let count = count_col.u32()?.get(row).unwrap_or(0);
            let percentage = (count as f64 / total as f64) * 100.0;
            println!("{:<20} | {:>5} | {:>8.1}%", method, count, percentage);
        }
        
        println!("\nTotal Discovery Methods: {}", discovery_methods_df.height());
    }
            ...
            if let Some(series) = col.as_series() {
                if let Ok(f64_series) = series.f64() {
                    ...
                    if count > 0 {
                        ...
                        let min = f64_series.into_iter().filter_map(|x| x).fold(f64::INFINITY, |a, b| a.min(b));
                        let max = f64_series.into_iter().filter_map(|x| x).fold(f64::NEG_INFINITY, |a, b| a.max(b));
    ...
    println!("HD         |   {:>5.1}% | {}", 
        ...
        (catalog_data.hd_match_rate * catalog_data.total_stars as f64 / 100.0) as u32);
    println!("HIP        |   {:>5.1}% | {}", 
        catalog_data.hip_match_rate, 
        (catalog_data.hip_match_rate * catalog_data.total_stars as f64 / 100.0) as u32);
    println!("TIC        |   {:>5.1}% | {}", 
        catalog_data.tic_match_rate, 
        (catalog_data.tic_match_rate * catalog_data.total_stars as f64 / 100.0) as u32);
    println!("GAIA DR2   |   {:>5.1}% | {}", 
        catalog_data.gaia_dr2_match_rate, 
        (catalog_data.gaia_dr2_match_rate * catalog_data.total_stars as f64 / 100.0) as u32);
    println!("GAIA DR3   |   {:>5.1}% | {}", 
        catalog_data.gaia_dr3_match_rate, 
        (catalog_data.gaia_dr3_match_rate * catalog_data.total_stars as f64 / 100.0) as u32);
    ...
    println!("\nTotal Bands: {} | Computed in {:?}", 
        photo_data.band_stats.len(), photo_time);
    println!();
    ...
    println!("Memory per Star: {:.2} KB", memory_mb * 1024.0 / df.height() as f64);
    ...
    let total_computation = temp_time + discovery_time + catalog_time + photo_time;
