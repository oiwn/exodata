...
```bash
cargo run --package exo-cli -- convert-raw-files
```

Or with a custom data directory:

```bash
cargo run --package exo-cli -- convert-raw-files --data-dir path/to/data
```
...
cargo run --package exo-cli -- view-fields --path data/stellarhosts.vot

# View column metadata (units, descriptions)
cargo run --package exo-cli -- view-metadata --path data/exoplanets.vot

# Sample rows from parquet
cargo run --package exo-cli -- view-samples
cargo run --package exo-cli -- view-exoplanets-samples

# Statistics
cargo run --package exo-cli -- view-stats
cargo run --package exo-cli -- view-exoplanets-stats

# Run SQL against parquet files (tables: stellarhosts, exoplanets)
cargo run --package exo-cli -- sql "SELECT pl_name, pl_orbper FROM exoplanets LIMIT 10"
```
