...
pub fn temperature_distribution(df: &DataFrame) -> Result<Vec<TemperatureBin>> {
    ...
    let count_column = |col_name: &str| -> Result<u32> {
        if let Ok(col) = df.column(col_name) {
            if let Some(series) = col.as_series() {
                return Ok((series.len() - series.null_count()) as u32);
            }
        }
        Ok(0)
    };
...
pub fn photometric_statistics(df: &DataFrame) -> Result<PhotometricStats> {
...
fn compute_band_stats(df: &DataFrame, column: &str) -> Result<BandStats> {
